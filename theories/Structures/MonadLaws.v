...
Section MonadLaws.
   ...
   **)
  Variable meq : forall {T : Type}, type T -> type (m T).
  Variable meqOk : forall {T : Type} (tT : type T), typeOk tT -> typeOk (meq tT).
...
*)
  ...
  { bind_of_return : forall {A B : Type} (eA : type A) (eB : type B),
    typeOk eA -> typeOk eB ->
    forall (a:A) (f:A -> m B),
    proper a -> proper f ->
    equal (bind (ret a) f) (f a)
  ; return_of_bind : forall {A : Type} (eA : type A),
    typeOk eA ->
    ...
    equal (bind aM f) aM
  ; bind_associativity :
    forall {A B C : Type} (eA : type A) (eB : type B) (eC : type C),
  ...
  ; ret_proper :> forall {A:Type} (eA : type A), typeOk eA ->
    proper ret
  ; bind_proper :> forall {A B:Type} (eA : type A) (eB : type B),
    typeOk eA -> typeOk eB ->
    proper (@bind m _ A B)
  }.
...
*)
  ...
  { lift_ret  : forall {T : Type} (eT : type T),
    typeOk eT ->
    forall x : T,
    proper x ->
    equal (lift (ret x)) (ret x)
  ; lift_bind : forall {T U : Type} (eT : type T) (eU : type U),
    typeOk eT -> typeOk eU ->
    forall (c : n T) (f : T -> n U),
    proper c -> proper f ->
    equal (lift (bind c f)) (bind (lift c) (fun x => lift (f x)))
  ; lift_proper : forall {T : Type} (tT : type T), typeOk tT -> proper lift
  }.
...
(*
    Definition promote {A : Type} : Type := A.
...
*)
  ...
  { bind_zero : forall {A B:Type} (tA : type A) (tB : type B),
    typeOk tA -> typeOk tB ->
    forall (f : A -> m B),
    proper f ->
    equal (bind mzero f) mzero
  ; zero_proper :> forall {A:Type} (rA : type A), typeOk rA ->
    proper mzero
  }.

  Class MonadFixLaws (MF : MonadFix m) : Type :=
  { mleq : forall {T:Type}, relation T -> relation (m T)
  ; mfix_monotonic : forall {T U:Type} (rT : type T) (rU : type U),
    typeOk rT -> typeOk rU ->
    forall (F : (T -> m U) -> T -> m U),
    respectful equal (mleq equal) (mfix F) (F (mfix F))
  ; mfix_proper :> forall {T U:Type} (rT : type T) (rU : type U),
    typeOk rT -> typeOk rU ->
    proper (@mfix _ _ T U)
  }.
